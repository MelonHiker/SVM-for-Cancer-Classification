...
//	int *perm = Malloc(int,prob->l);
    int *perm_svc=nullptr;
	double *dec_values = Malloc(double,prob->l);

//    if(run_times<2)
    {
        first_run=false;
        perm_svc_group[run_times] = Malloc(int,prob->l);
        perm_svc=perm_svc_group[run_times];
        for(i=0;i<prob->l;i++) perm_svc[i]=i;
        for(i=0;i<prob->l;i++)
        {
            int j = i+rand()%(prob->l-i);
            swap(perm_svc[i],perm_svc[j]);
        }
        
    }
//    perm_svc=perm_svc_group[run_times % 2];
//    run_times++;
//    if(run_times>=1000)
//    {
//        run_times-=1000;
//    }
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		for(j=0;j<begin;j++)
		{
			subprob.x[k] = prob->x[perm_svc[j]];
			subprob.y[k] = prob->y[perm_svc[j]];
			++k;
		}
		...
		if(p_count==0 && n_count==0)
			for(j=begin;j<end;j++)
				dec_values[perm_svc[j]] = 0;
		else if(p_count > 0 && n_count == 0)
			for(j=begin;j<end;j++)
				dec_values[perm_svc[j]] = 1;
		else if(p_count == 0 && n_count > 0)
			for(j=begin;j<end;j++)
				dec_values[perm_svc[j]] = -1;
		else
		{
			...
			for(j=begin;j<end;j++)
			{
				svm_predict_values(submodel,prob->x[perm_svc[j]],&(dec_values[perm_svc[j]]));
				// ensure +1 -1 order; reason not using CV subroutine
				dec_values[perm_svc[j]] *= submodel->label[0];
			}
...
//	int nr_class;
	...
	for(i=0;i<nr_fold;i++)
	{
		   ...
		   (param->svm_type == C_SVC || param->svm_type == NU_SVC))
		{
			...
			for(j=begin;j<end;j++)
            {
                target[perm[j]*2] = svm_predict_probability(submodel,prob->x[perm[j]],prob_estimates);
                target[perm[j]*2+1]=prob_estimates[0];
 //               printf("%f , %f \n", prob_estimates[0], prob_estimates[1]);
			...
			for(j=begin;j<end;j++)
            {
                target[perm[j]*2] = svm_predict(submodel,prob->x[perm[j]]);
                target[perm[j]*2+1]=0;
            }
		svm_free_and_destroy_model(&submodel);
		free(subprob.x);
